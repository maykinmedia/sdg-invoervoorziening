...
urlpatterns = [
    path(
        "sdg/admin/password_reset/",
        PasswordResetView.as_view(),
        name="admin_password_reset",
    ),
    path(
        "sdg/admin/password_reset/done/",
        auth_views.PasswordResetDoneView.as_view(),
        name="password_reset_done",
    ),
    path("sdg/admin/hijack/", include("hijack.urls")),
    path("sdg/admin/", admin.site.urls),
    path("sdg/markdownx/", include("markdownx.urls")),
    path(
        "reset/<uidb64>/<token>/",
    ...
    url(
        r"^sdg/accept-invitation/(?P<key>\w+)/?$",
...
if settings.DEBUG and apps.is_installed("debug_toolbar"):
    ...
    urlpatterns = [
        path("sdg/__debug__/", include(debug_toolbar.urls)),
    ] + urlpatterns

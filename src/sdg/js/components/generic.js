...
class GenericForm {
    ...
    resetSpecefiekeGegevens() {
        ...
        const markdown_editor_fields = document.querySelectorAll(
            ".markdownx textarea"
        );
            ...
            const displayFunc = (displayDependency) => {
                ...
                if (input.selectedIndex === 2) {
                    ...
                    controls.forEach((element) => {
                        ...
                        if (!textarea.value) {
                            const defaultExplanation = document.querySelector(
                                `.form__reference-${element.lang}`
                            ).dataset.productAanwezigToelichting;
            ...
            const displayFunc = async (select, displayDependency) => {
                if (select.selectedIndex > 0) {
                    ...
                    for (const element of controls) {
                        ...
                        const defaultExplanation = document.querySelector(
                            `.form__reference-${element.lang}`
                        ).dataset.defaultToelichting;
                        const explanation = defaultExplanation.replace(
                            /\[product\]/g,
                            await this.getProductTranslationName(
                                select.value,
                                element.lang
                            )
                        );

# Generated by Django 2.2.24 on 2021-09-15 15:42

import django.contrib.postgres.fields
from django.db import migrations, models
import django.db.models.deletion
import markdownx.models
import sdg.core.db.fields


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ("core", "0001_initial"),
        ("organisaties", "0001_initial"),
    ]

    operations = [
        migrations.CreateModel(
            name="GeneriekProduct",
            fields=[
                (
                    "id",
                    models.AutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                (
                    "upn",
                    models.ForeignKey(
                        help_text="De uniforme productnaam met betrekking tot dit product.",
                        on_delete=django.db.models.deletion.PROTECT,
                        related_name="generiek_product",
                        to="core.UniformeProductnaam",
                        verbose_name="uniforme productnaam",
                    ),
                ),
            ],
            options={
                "verbose_name": "generiek product",
                "verbose_name_plural": "generiek product",
            },
        ),
        migrations.CreateModel(
            name="Productuitvoering",
            fields=[
                (
                    "id",
                    models.AutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
            ],
            options={
                "verbose_name": "specifiek product",
                "verbose_name_plural": "specifiek product",
            },
        ),
        migrations.CreateModel(
            name="ReferentieProduct",
            fields=[
                (
                    "id",
                    models.AutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                (
                    "generiek",
                    models.OneToOneField(
                        help_text="Het generieke moederproduct voor deze referentie.",
                        on_delete=django.db.models.deletion.PROTECT,
                        related_name="referentie",
                        to="producten.GeneriekProduct",
                        verbose_name="generiek",
                    ),
                ),
            ],
            options={
                "verbose_name": "referentie product",
                "verbose_name_plural": "referentie product",
            },
        ),
        migrations.CreateModel(
            name="SpecifiekProduct",
            fields=[
                (
                    "id",
                    models.AutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                (
                    "doelgroep",
                    sdg.core.db.fields.ChoiceArrayField(
                        base_field=models.CharField(
                            choices=[
                                ("burgers", "Burgers"),
                                ("bedrijven", "Bedrijven"),
                            ],
                            max_length=32,
                        ),
                        blank=True,
                        default=list,
                        help_text="Geeft aan voor welke doelgroep het product is bedoeld: burgers, bedrijven of burgers en bedrijven. Wordt gebruikt wanneer een portaal informatie over het product ophaalt uit de invoervoorziening. Zo krijgen de ondernemersportalen de ondernemersvariant en de burgerportalen de burgervariant. ",
                        size=None,
                    ),
                ),
                (
                    "beschikbaar",
                    models.BooleanField(
                        help_text="Geeft aan of het product al dan niet beschikbaar is.",
                        verbose_name="beschikbaar",
                    ),
                ),
                (
                    "versie",
                    models.PositiveIntegerField(
                        default=1,
                        help_text="Het versienummer van het item.",
                        verbose_name="versie",
                    ),
                ),
                (
                    "publicatie_datum",
                    models.DateTimeField(
                        help_text="De datum van publicatie van de productspecifieke informatie.",
                        verbose_name="publicatie datum",
                    ),
                ),
                (
                    "catalogus",
                    models.ForeignKey(
                        help_text="Referentie naar de catalogus waartoe dit product behoort.",
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="producten",
                        to="core.ProductenCatalogus",
                        verbose_name="catalogus",
                    ),
                ),
                (
                    "gerelateerd_product",
                    models.ForeignKey(
                        blank=True,
                        help_text="Een verwijzing naar een gerelateerd product.",
                        null=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="gerelateerd",
                        to="producten.SpecifiekProduct",
                        verbose_name="gerelateerd aan",
                    ),
                ),
                (
                    "lokaties",
                    models.ManyToManyField(
                        help_text="De locaties die zijn toegewezen aan de product.",
                        related_name="producten",
                        to="organisaties.Lokatie",
                        verbose_name="lokaties",
                    ),
                ),
                (
                    "product",
                    models.OneToOneField(
                        blank=True,
                        help_text="Een verwijzing naar een ander product.",
                        null=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="gerefereerd",
                        to="producten.SpecifiekProduct",
                        verbose_name="refereert aan",
                    ),
                ),
                (
                    "referentie",
                    models.ForeignKey(
                        help_text="Het referentieproduct voor het specifieke product.",
                        on_delete=django.db.models.deletion.PROTECT,
                        related_name="specifiek",
                        to="producten.ReferentieProduct",
                        verbose_name="referentie",
                    ),
                ),
            ],
            options={
                "verbose_name": "specifiek product",
                "verbose_name_plural": "specifiek product",
            },
        ),
        migrations.CreateModel(
            name="ProductuitvoeringInformatie",
            fields=[
                (
                    "id",
                    models.AutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                (
                    "taal",
                    models.CharField(
                        blank=True,
                        choices=[
                            ("Nederlands", "Nederlands"),
                            ("Engels", "Engels"),
                            ("Duits", "Duits"),
                        ],
                        help_text="De taal waarin de betreffende tekst is geschreven.ISO 639 (https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes)",
                        max_length=32,
                        null=True,
                        verbose_name="taal",
                    ),
                ),
                (
                    "product_titel_decentraal",
                    models.CharField(
                        blank=True,
                        help_text="De titel van het decentrale product, die immers kan afwijken van de landelijke titel.",
                        max_length=50,
                        null=True,
                        verbose_name="product titel decentraal",
                    ),
                ),
                (
                    "specifieke_tekst",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Decentrale omschrijving.",
                        null=True,
                        verbose_name="specifieke tekst",
                    ),
                ),
                (
                    "verwijzing_links",
                    django.contrib.postgres.fields.ArrayField(
                        base_field=models.URLField(
                            max_length=1000, verbose_name="url van verwijzing"
                        ),
                        blank=True,
                        default=list,
                        help_text="Decentrale verwijzingen.",
                        null=True,
                        size=None,
                    ),
                ),
                (
                    "specifieke_link",
                    models.URLField(
                        blank=True,
                        help_text="URL decentrale productpagina.",
                        null=True,
                        verbose_name="specifieke link",
                    ),
                ),
                (
                    "decentrale_link",
                    models.URLField(
                        blank=True,
                        help_text="Link naar decentrale productpagina voor burgers en / of bedrijven.",
                        null=True,
                        verbose_name="decentrale link",
                    ),
                ),
                (
                    "datum_wijziging",
                    models.DateTimeField(
                        blank=True,
                        help_text="Decentrale overheden geven een wijzigingsdatum mee voor hun informatie. Deze datum wordt op het portaal getoond. ",
                        null=True,
                        verbose_name="datum wijziging",
                    ),
                ),
                (
                    "procedure_beschrijving",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Procedurebeschrijving.",
                        null=True,
                        verbose_name="procedure beschrijving",
                    ),
                ),
                (
                    "vereisten",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Vereisten auth/id/sign.",
                        null=True,
                        verbose_name="vereisten",
                    ),
                ),
                (
                    "bewijs",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Bewijs (type/format).",
                        null=True,
                        verbose_name="bewijs",
                    ),
                ),
                (
                    "bezwaar_en_beroep",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Bezwaar en beroep.",
                        null=True,
                        verbose_name="bezwaar en beroep",
                    ),
                ),
                (
                    "kosten_en_betaalmethoden",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Kosten en betaalmethoden.",
                        null=True,
                        verbose_name="kosten en betaalmethoden",
                    ),
                ),
                (
                    "uiterste_termijn",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Deadlines.",
                        null=True,
                        verbose_name="uiterste termijn",
                    ),
                ),
                (
                    "wtd_bij_geen_reactie",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Wat te doen bij geen reactie.",
                        null=True,
                        verbose_name="wtd bij geen reactie",
                    ),
                ),
                (
                    "decentrale_procedure_link",
                    models.URLField(
                        blank=True,
                        help_text="Link naar de procedure voor burgers en / of bedrijven.",
                        null=True,
                        verbose_name="decentrale procedure link",
                    ),
                ),
                (
                    "productuitvoering",
                    models.ForeignKey(
                        help_text="Het moeder-productuitvoering van deze informatie.",
                        on_delete=django.db.models.deletion.PROTECT,
                        related_name="informatie",
                        to="producten.Productuitvoering",
                        verbose_name="productuitvoering",
                    ),
                ),
            ],
            options={
                "verbose_name": "product specifiek informatie",
                "verbose_name_plural": "product specifiek informatie",
            },
        ),
        migrations.AddField(
            model_name="productuitvoering",
            name="specifiek_product",
            field=models.ForeignKey(
                help_text="Het referentieproduct voor het specifieke product.",
                on_delete=django.db.models.deletion.PROTECT,
                related_name="uitvoeringen",
                to="producten.SpecifiekProduct",
                verbose_name="referentie",
            ),
        ),
        migrations.CreateModel(
            name="ProductSpecifiekInformatie",
            fields=[
                (
                    "id",
                    models.AutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                (
                    "taal",
                    models.CharField(
                        blank=True,
                        choices=[
                            ("Nederlands", "Nederlands"),
                            ("Engels", "Engels"),
                            ("Duits", "Duits"),
                        ],
                        help_text="De taal waarin de betreffende tekst is geschreven.ISO 639 (https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes)",
                        max_length=32,
                        null=True,
                        verbose_name="taal",
                    ),
                ),
                (
                    "product_titel_decentraal",
                    models.CharField(
                        blank=True,
                        help_text="De titel van het decentrale product, die immers kan afwijken van de landelijke titel.",
                        max_length=50,
                        null=True,
                        verbose_name="product titel decentraal",
                    ),
                ),
                (
                    "specifieke_tekst",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Decentrale omschrijving.",
                        null=True,
                        verbose_name="specifieke tekst",
                    ),
                ),
                (
                    "verwijzing_links",
                    django.contrib.postgres.fields.ArrayField(
                        base_field=models.URLField(
                            max_length=1000, verbose_name="url van verwijzing"
                        ),
                        blank=True,
                        default=list,
                        help_text="Decentrale verwijzingen.",
                        null=True,
                        size=None,
                    ),
                ),
                (
                    "specifieke_link",
                    models.URLField(
                        blank=True,
                        help_text="URL decentrale productpagina.",
                        null=True,
                        verbose_name="specifieke link",
                    ),
                ),
                (
                    "decentrale_link",
                    models.URLField(
                        blank=True,
                        help_text="Link naar decentrale productpagina voor burgers en / of bedrijven.",
                        null=True,
                        verbose_name="decentrale link",
                    ),
                ),
                (
                    "datum_wijziging",
                    models.DateTimeField(
                        blank=True,
                        help_text="Decentrale overheden geven een wijzigingsdatum mee voor hun informatie. Deze datum wordt op het portaal getoond. ",
                        null=True,
                        verbose_name="datum wijziging",
                    ),
                ),
                (
                    "procedure_beschrijving",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Procedurebeschrijving.",
                        null=True,
                        verbose_name="procedure beschrijving",
                    ),
                ),
                (
                    "vereisten",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Vereisten auth/id/sign.",
                        null=True,
                        verbose_name="vereisten",
                    ),
                ),
                (
                    "bewijs",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Bewijs (type/format).",
                        null=True,
                        verbose_name="bewijs",
                    ),
                ),
                (
                    "bezwaar_en_beroep",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Bezwaar en beroep.",
                        null=True,
                        verbose_name="bezwaar en beroep",
                    ),
                ),
                (
                    "kosten_en_betaalmethoden",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Kosten en betaalmethoden.",
                        null=True,
                        verbose_name="kosten en betaalmethoden",
                    ),
                ),
                (
                    "uiterste_termijn",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Deadlines.",
                        null=True,
                        verbose_name="uiterste termijn",
                    ),
                ),
                (
                    "wtd_bij_geen_reactie",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Wat te doen bij geen reactie.",
                        null=True,
                        verbose_name="wtd bij geen reactie",
                    ),
                ),
                (
                    "decentrale_procedure_link",
                    models.URLField(
                        blank=True,
                        help_text="Link naar de procedure voor burgers en / of bedrijven.",
                        null=True,
                        verbose_name="decentrale procedure link",
                    ),
                ),
                (
                    "specifiek_product",
                    models.ForeignKey(
                        help_text="Het specifiek moederproduct van deze informatie.",
                        on_delete=django.db.models.deletion.PROTECT,
                        related_name="informatie",
                        to="producten.SpecifiekProduct",
                        verbose_name="specifiek product",
                    ),
                ),
            ],
            options={
                "verbose_name": "product specifiek informatie",
                "verbose_name_plural": "product specifiek informatie",
            },
        ),
        migrations.CreateModel(
            name="ProductReferentieInformatie",
            fields=[
                (
                    "id",
                    models.AutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                (
                    "taal",
                    models.CharField(
                        choices=[
                            ("Nederlands", "Nederlands"),
                            ("Engels", "Engels"),
                            ("Duits", "Duits"),
                        ],
                        help_text="De taal waarin de betreffende tekst is geschreven.ISO 639 (https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes)",
                        max_length=32,
                        verbose_name="taal",
                    ),
                ),
                (
                    "product_titel_decentraal",
                    models.CharField(
                        help_text="De titel van het decentrale product, die immers kan afwijken van de landelijke titel.",
                        max_length=50,
                        verbose_name="product titel decentraal",
                    ),
                ),
                (
                    "specifieke_tekst",
                    markdownx.models.MarkdownxField(
                        help_text="Decentrale omschrijving.",
                        verbose_name="specifieke tekst",
                    ),
                ),
                (
                    "verwijzing_links",
                    django.contrib.postgres.fields.ArrayField(
                        base_field=models.URLField(
                            max_length=1000, verbose_name="url van verwijzing"
                        ),
                        blank=True,
                        default=list,
                        help_text="Decentrale verwijzingen.",
                        size=None,
                    ),
                ),
                (
                    "specifieke_link",
                    models.URLField(
                        help_text="URL decentrale productpagina.",
                        verbose_name="specifieke link",
                    ),
                ),
                (
                    "decentrale_link",
                    models.URLField(
                        help_text="Link naar decentrale productpagina voor burgers en / of bedrijven.",
                        verbose_name="decentrale link",
                    ),
                ),
                (
                    "datum_wijziging",
                    models.DateTimeField(
                        help_text="Decentrale overheden geven een wijzigingsdatum mee voor hun informatie. Deze datum wordt op het portaal getoond. ",
                        verbose_name="datum wijziging",
                    ),
                ),
                (
                    "procedure_beschrijving",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Procedurebeschrijving.",
                        null=True,
                        verbose_name="procedure beschrijving",
                    ),
                ),
                (
                    "vereisten",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Vereisten auth/id/sign.",
                        null=True,
                        verbose_name="vereisten",
                    ),
                ),
                (
                    "bewijs",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Bewijs (type/format).",
                        null=True,
                        verbose_name="bewijs",
                    ),
                ),
                (
                    "bezwaar_en_beroep",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Bezwaar en beroep.",
                        null=True,
                        verbose_name="bezwaar en beroep",
                    ),
                ),
                (
                    "kosten_en_betaalmethoden",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Kosten en betaalmethoden.",
                        null=True,
                        verbose_name="kosten en betaalmethoden",
                    ),
                ),
                (
                    "uiterste_termijn",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Deadlines.",
                        null=True,
                        verbose_name="uiterste termijn",
                    ),
                ),
                (
                    "wtd_bij_geen_reactie",
                    markdownx.models.MarkdownxField(
                        blank=True,
                        help_text="Wat te doen bij geen reactie.",
                        null=True,
                        verbose_name="wtd bij geen reactie",
                    ),
                ),
                (
                    "decentrale_procedure_link",
                    models.URLField(
                        blank=True,
                        help_text="Link naar de procedure voor burgers en / of bedrijven.",
                        null=True,
                        verbose_name="decentrale procedure link",
                    ),
                ),
                (
                    "referentie_product",
                    models.ForeignKey(
                        help_text="Het referentie moederproduct van deze informatie.",
                        on_delete=django.db.models.deletion.PROTECT,
                        related_name="informatie",
                        to="producten.ReferentieProduct",
                        verbose_name="referentie product",
                    ),
                ),
            ],
            options={
                "verbose_name": "product referentie informatie",
                "verbose_name_plural": "product referentie informatie",
            },
        ),
        migrations.CreateModel(
            name="ProductGeneriekInformatie",
            fields=[
                (
                    "id",
                    models.AutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                (
                    "taal",
                    models.CharField(
                        choices=[
                            ("Nederlands", "Nederlands"),
                            ("Engels", "Engels"),
                            ("Duits", "Duits"),
                        ],
                        help_text="De taal waarin de betreffende tekst is geschreven.ISO 639 (https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes)",
                        max_length=32,
                        verbose_name="taal",
                    ),
                ),
                (
                    "product_titel",
                    models.CharField(
                        help_text="De titel van het decentrale product, die immers kan afwijken van de landelijke titel.",
                        max_length=50,
                        verbose_name="product titel",
                    ),
                ),
                (
                    "generieke_tekst",
                    markdownx.models.MarkdownxField(
                        help_text="De Nationale Portalen schrijven een inleidende, algemene tekst over het product. Het idee is dat deze ",
                        verbose_name="generieke tekst",
                    ),
                ),
                (
                    "korte_omschrijving",
                    models.CharField(
                        help_text='Korte omschrijving van wat er op de pagina staat, gebruikt in de meta tags van de productpagina (meta name="description"). Deze tekst wordt gebruikt om te tonen wanneer de pagina wordt gevonden in een zoekmachine. ',
                        max_length=80,
                        verbose_name="korte omschrijving",
                    ),
                ),
                (
                    "datum_check",
                    models.DateTimeField(
                        help_text="De informatie over het product kan worden gewijzigd en gecontroleerd op actualiteit (gecheckt). De Nationale portalen houden bij wanneer de informatie voor het laasts is 'gecheckt'.  Deze datum wordt op het portaal getoond.",
                        verbose_name="datum check",
                    ),
                ),
                (
                    "verwijzing_links",
                    django.contrib.postgres.fields.ArrayField(
                        base_field=models.URLField(
                            max_length=1000, verbose_name="url van verwijzing"
                        ),
                        blank=True,
                        default=list,
                        help_text="Zowel de Nationale Portalen als de decentrale overheden kunnen een x-tal 'verwijzingen' opnemen bij een product. Voorstel hierbij om zo'n 'verwijzing' te laten bestaan uit een -bij elkaar horende-  beschrijving en hyperlink. De tekst van artikel 9 geeft enkele voorbeelden hiervan: 'richtsnoeren/ NEN-specs bijv en ook links naar de wetgeving (dit laatste is inderdaad te verstaan als vertaling van 'Legal Acts'. Bart heeft dit bij Jurist van bNC (Vanessa) gecheckt. ",
                        size=None,
                    ),
                ),
                (
                    "landelijke_link",
                    models.URLField(
                        help_text="URL van de productpagina wanneer het een landelijk product betreft of de pagina met enkel generieke beschrijving van een decentraal product, bijvoorbeeld : https://ondernemersplein.kvk.nl/terrasvergunning. gebruikt voor o.a. notificeren, feedback & statistics en het kunnen bekijken van de generieke productinformatie (bv door gebruikers van de gemeentelijke invoervoorziening) ",
                        verbose_name="landelijke link",
                    ),
                ),
                (
                    "verplicht_product",
                    models.BooleanField(
                        help_text="Geeft aan of decentrale overheden verplicht zijn informatie over dit product te leveren.",
                        verbose_name="verplicht product",
                    ),
                ),
                (
                    "generieke_link",
                    models.URLField(
                        help_text="URL van de productpagina wanneer het een landelijk product betreft of de pagina met enkel generieke beschrijving van een decentraal product, bijvoorbeeld : https://ondernemersplein.kvk.nl/terrasvergunning. gebruikt voor o.a. notificeren, feedback & statistics en het kunnen bekijken van de generieke productinformatie (bv door gebruikers van de gemeentelijke invoervoorziening) ",
                        verbose_name="generieke link",
                    ),
                ),
                (
                    "generiek_product",
                    models.ForeignKey(
                        help_text="Het generieke moederproduct van deze informatie.",
                        on_delete=django.db.models.deletion.PROTECT,
                        related_name="informatie",
                        to="producten.GeneriekProduct",
                        verbose_name="generiek product",
                    ),
                ),
                (
                    "verantwoordelijke_organisatie",
                    models.ForeignKey(
                        help_text="Organisatie verantwoordelijk voor de landelijke informatie",
                        on_delete=django.db.models.deletion.PROTECT,
                        related_name="generiek_informatie",
                        to="core.Overheidsorganisatie",
                        verbose_name="verantwoordelijke organisatie",
                    ),
                ),
            ],
            options={
                "verbose_name": "product generiek informatie",
                "verbose_name_plural": "product generiek informatie",
            },
        ),
    ]
